...
std::tuple<Matrix, Matrix, Matrix> Decomposer::SVD(const Matrix& A) {
    ...
    Logger::getInstance().log("Note: This method squares condition number and may lose precision for ill-conditioned matrices");
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int k = 0; k < n; ++k) {
            V_sorted(k, i) = V(k, oldIndex);
        }
        
        // Calculate U columns: u_i = A * v_i / sigma
        if (sigma > 1e-10) {
            std::vector<double> v_col(n);
            for(int k=0; k<n; ++k) v_col[k] = V_sorted(k, i);
            
            // Multiply A * v_col
            std::vector<double> u_col(m, 0.0);
            for(int r=0; r<m; ++r) {
                for(int c=0; c<n; ++c) {
                    u_col[r] += A(r, c) * v_col[c];
                }
            }
            
            for(int k=0; k<m; ++k) U(k, i) = u_col[k] / sigma;
        }
    }
